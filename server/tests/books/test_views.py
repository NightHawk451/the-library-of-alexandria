...
def test_add_book(client):
    ...
    resp = client.post(
        "/api/books/",
        {"title": "The Big Lebowski", "author": "comedy", "year": "1998"},
...
def test_add_book_invalid_json(client):
    ...
    resp = client.post("/api/books/", {}, content_type="application/json")
...
def test_add_book_invalid_json_keys(client):
    ...
    resp = client.post(
        "/api/books/",
        {"title": "The Big Lebowski", "author": "comedy"},
        content_type="application/json",
    )
    assert resp.status_code == 400
...
def test_get_single_book(client, add_book):
    book = add_book(title="The Big Lebowski", author="comedy", year="1998")
...
def test_get_single_book_incorrect_id(client):
    resp = client.get("/api/books/foo/")
    assert resp.status_code == 404


@pytest.mark.django_db
def test_get_all_books(client, add_book):
    book_one = add_book(title="The Big Lebowski", author="comedy", year="1998")
